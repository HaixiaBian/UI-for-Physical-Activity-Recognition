 ...
 * unmovable, undeletable activity block.
 */
...
BlockFactory.STARTER_BLOCK_XML_TEXT =
    '<xml xmlns="https://developers.google.com/blockly/xml">' +
    '<block type="activity" deletable="false" movable="false">' +
    '<field name="NAME">new_activity</field>' +
    '</block></xml>';
...
BlockFactory.updatePreview = function() {
  ...
  // collide if user creates a 'activity' block, for instance.
  ...
  try {
    ...
    } else if (blockType == 'new_activity') {
...
BlockFactory.isStarterBlock = function() {
  var rootBlock = FactoryUtils.getRootBlock(BlockFactory.mainWorkspace);
  return rootBlock && !(
      // The starter block does not have blocks nested into the activity block.
      rootBlock.getChildren().length > 0 ||
      // The starter block's name is the default, 'new_activity'.
      rootBlock.getFieldValue('NAME').trim().toLowerCase() != 'new_activity' ||
