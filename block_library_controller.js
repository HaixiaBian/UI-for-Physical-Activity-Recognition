...
/**
 * Lists block types in library.
 * @return {!Array.<string>} Array of block types stored in library.
 */
BlockLibraryController.prototype.getBlockTypesInLibrary = function() {
 ...
 * updating the dropdown and displaying the starter block (activity).
 */
...
BlockLibraryController.prototype.saveToBlockLibrary = function() {
  ...
  if (blockType == 'new_activity') {
    // Do not save block if it has the default type, 'new_activity'.
    var msg = 'You cannot save a block under the name "new_activity". Try ' +
